...
let b;
const goto=async(u)=>{await b.send("Page.navigate",{url:u});for(let i=0;i<40;i++){await sleep(250);
  if(await b.ev(`document.getElementById("root")?.childElementCount>0`).catch(()=>false))break;} await sleep(2200)};
const state=()=>b.ev(`(async()=>{const m=await import("/src/lib/store.js");const p=await m.getPlayer();
  return {uid:p.uid,nickname:p.nickname,classCode:p.classCode,xp:p.xp,cau:Object.keys(p.answers||{}).length};})()`);
const fillForm=(sel,nick,cls)=>b.ev(`(()=>{const els=document.querySelectorAll("${sel} input");
...
try{
  b=await openBrave("390,2200");

  console.log(`\n[1] Vào lớp ${CU} và kiếm XP`);
  await goto(`${BASE}/c/N01`);
  await fillForm(".gate","Alice",CU);
  await b.ev(`document.querySelector(".gate button[type=submit]").click()`);await sleep(3000);
  ...
  const daTraLoi = await b.ev(`(async()=>{
  ...
  await goto(`${BASE}/`);
  await b.ev(`window.__hoi=0; window.confirm=(m)=>{window.__hoi++;window.__msg=m;return true;};`);
  await fillForm("form.form","AliceHai",CU);
  await b.ev(`document.querySelector("form.form button[type=submit]").click()`);await sleep(3500);
  const s2=await state();console.log("   ",JSON.stringify(s2));
  check("Đổi tên KHÔNG hỏi lại", (await b.ev(`window.__hoi`))===0);
  ...
  await goto(`${BASE}/`);
  await b.ev(`window.__hoi=0; window.__msg=""; window.confirm=(m)=>{window.__hoi++;window.__msg=m;return true;};`);
  await fillForm("form.form","Bob",MOI);
  await b.ev(`document.querySelector("form.form button[type=submit]").click()`);await sleep(4000);
  const hoi=await b.ev(`window.__hoi`); const msg=await b.ev(`window.__msg`);
  ...
  const bCu=await b.ev(`[...document.querySelectorAll(".rank li")].map(l=>l.innerText.replace(/\\n/g," ")).join(" | ")||"(trống)"`);
  ...
  const tCu=await b.ev(`document.querySelector(".stats")?.innerText.replace(/\\n/g," ")||"(trống)"`);
  ...
  const bMoi=await b.ev(`[...document.querySelectorAll(".rank li")].map(l=>l.innerText.replace(/\\n/g," ")).join(" | ")||"(trống)"`);
